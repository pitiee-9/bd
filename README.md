...
2. Copy `.env.example` to `.env` and set `DATABASE_URL`, `BLOB_READ_WRITE_TOKEN`, `SESSION_SECRET`, and `ADMIN_PASSWORD`.
3. Run `npm install`.
...
Use HTTPS, a strong random `SESSION_SECRET`, a shared production session store, a reverse proxy, and Vercel project environment variables. Uploaded images are limited to JPEG, PNG, and WebP and 5 MB; image re-encoding and EXIF stripping should be added before accepting untrusted production uploads.
"# bd" 
